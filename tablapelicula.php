  ...
  <table style="background-color:white" class="table-sm table-bordered">
	...
	<thead>
			<tr class="text-sm-center">
				<th colspan="10">Lista de Peliculas</th>
			</tr>
		</thead>
	    <tbody >
	    	<tr>
		...
		<th class="text-sm-center">Genero</th>
		<th class="text-sm-center">Director</th>
		<th class="text-sm-center">Año</th>
		<th class="text-sm-center" colspan="2">Operaciones</th>
	        </tr>
	    	<?php 
         	include("conexion.php") ;
            $insertar = "SELECT * FROM pelicula" ;
            $resultado = $conexion-> query($insertar);
            while ($fila=$resultado->fetch_assoc()) {
           ?>	
        <tr>
     	<td> <?php echo $fila['id'];?></td>
     	<td> <?php echo $fila ['peli'];?></td>
     	<td> <?php echo $fila ['genero'];?></td>
     	<td> <?php echo $fila ['director'] ;?></td>
     	<td> <?php echo $fila ['anio'];?></td>
      <td> <a class="btn btn-info btn-sm " href="modificarpelicula.php?id=<?php echo $fila['id']; ?>"> Modificar</a></td>
     	<td> <a class="btn btn-danger btn-sm" href="eliminarpelicula.php?id=<?php echo $fila['id']; ?>"> Eliminar</a></td>
     	</tr>
            <?php 
 				}	
             ?>
...
<center>
  <br>
  <br>
 <a class="btn btn-success" href="formulariopelicula.php"> Agregar Pelicula</a> </center>
